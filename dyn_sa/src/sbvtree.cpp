...
byte *SBVTree::number_ones = new byte[(1<<block_size)];
...
void output_bitarray(ostream &os, uint_block *tab, ulong nb_bits, ulong start) {
  ulong current_block = start>>log_bs;
  ulong current_pos = start&(block_size-1);
  for (ulong i = 0; i < nb_bits; i++) {
    os << ((tab[current_block] & (1<<current_pos))!=0) ;
    current_pos++;
...
ulong SBVTree::array_copyleft(uint_block *tab_dest, uint_block *tab_src,  ulong start_src, ulong nb_bits_copied) {
  if (! nb_bits_copied)
    return 0;
  ulong current_cell_dest = 0;
  ulong current_cell_src = start_src >> log_bs;
  uint_block pos_cell_src = start_src & (block_size-1);
//   uint_block *tab_dest = *ptab_dest;
//   uint_block *tab_src = *ptab_src;
  uint_block remaining_bits = nb_bits_copied & (block_size-1);
  ulong nb_memb = (nb_bits_copied >> log_bs);
  ulong nb_bits_set = 0;
  ...
  ulong nb_slices = 2*nb_memb;
  if (lsb_mask == 0)
    nb_slices--;
  uint_block value;

  for (ulong i = 0; i < nb_slices; i++) {
    if ((i & 1)) {
...
ulong SBVTree::array_copyright(uint_block *tab_dest, uint_block *tab_src, ulong start_dest, ulong nb_bits_copied) {
  if (! nb_bits_copied)
    return 0;
  ulong final_dest = start_dest+nb_bits_copied-1;
  ulong current_cell_dest = final_dest/block_size;
  uint_block pos_cell_dest = start_dest & (block_size-1);
  uint_block pos_right_cell_dest = final_dest & (block_size-1);
  ulong current_cell_src = 0;
//   uint_block *tab_dest = *ptab_dest;
//   uint_block *tab_src = *ptab_src;
  ulong nb_memb = (nb_bits_copied >> log_bs)+ ((nb_bits_copied & (block_size-1)) != 0);
  ...
  if (current_cell_src > 0) {
    ...
    ulong tmp_rank = number_ones[tab_dest[current_cell_dest]];
    
    if (pos_right_cell_dest < pos_cell_dest) { 
      // the copy ends in the msb
      ulong tmp_msb_mask = ((1<<(pos_right_cell_dest+1))-1) << msb_shift;
  ...
  // destination cell.
  ulong keep_bits = (1<<pos_cell_dest)-1;
  nb_bits_set += number_ones[tab_src[current_cell_src] & lsb_mask];
  ...
  bool SBVTree::iterateGetBit(){
    return (iterateNode->blocks[iterateBlock] & (1 << iterateLocal)) != 0;
  }
  ...
  SBVNode *SBVTree::getNodeWithIthBit(size_t i, ulong *infos) {
    ...
    if (current_node == getNil()) {
      cerr << "prob getNodeWithIthBit : " << i << endl;
    }
  ...
  void SBVTree::printNode(SBVNode *n){
    ...
    if (n!=getNil()) {
      cout << "taille : " << n->myPositions << endl;
      for (uint i=0; i < n->myPositions; i++) {
        if (i % block_size == 0)
          cout << " ";
        cout << ((n->blocks[i>>log_bs] & (1 << (i & (block_size-1)))) != 0); 
      }
      cout << endl;
...
//       cerr << "i = " << i << ", n->blocks[i] = " << tmp_dbg << endl;
//       cerr << "number_ones: " << (int) number_ones[tmp_dbg] << endl;
// #endif
  ...
  void SBVTree::deleteBit(size_t i){
    ...
    if (x->myPositions == 0){ 
    ...
    } else if (x->myPositions <= MIN_MERGE*superblock_size) // merge (and rotate), if necessary:
      {
        ...
        ulong nbPositionsRight=0, nbPositionsLeft=0;
        ...
        */ 
        if (nbPositionsRight
            && (merge_n_split = (x->myPositions+nbPositionsRight >= MAX_AFTER_MERGE*superblock_size 
                                 ...
                                 && (nbPositionsRight <= nbPositionsLeft || nbPositionsLeft == 0))
                || (merge = (nbPositionsRight+x->myPositions < MAX_AFTER_MERGE*superblock_size 
                             && (nbPositionsLeft >= nbPositionsRight || nbPositionsLeft == 0))))) {
          // Merging
          if (merge) {
            //shift the bits in the right sibling
            array_copyright((rightSibling->blocks), (rightSibling->blocks)
                            , x->myPositions, rightSibling->myPositions);
            // put all the bits from the current node in the right sibling
            array_copyright((rightSibling->blocks)
                            , (x->blocks)
                            , 0, x->myPositions);
            ...
            ulong gain = (rightSibling->myPositions-x->myPositions) >> 1;
            rank = array_copyright((x->blocks)
                                   , (rightSibling->blocks)
                                   , x->myPositions, gain);
            // Shift the bits in right sibling to discard the ones we just
            // copied in x.
            array_copyleft((rightSibling->blocks)
                           , (rightSibling->blocks)
                           , gain, rightSibling->myPositions-gain);
...
//                                    && ((nbPositionsLeft+x->myPositions) <= (2*superblock_size*MAX_AFTER_MERGE))
//                                    && nbPositionsRight < nbPositionsLeft)
//                   || (merge = (nbPositionsLeft+x->myPositions < MAX_AFTER_MERGE*superblock_size 
//                                && nbPositionsLeft < nbPositionsRight)))) {
            // Merging
            if (merge) {
              // put all the bits from the current node in the left sibling
              array_copyright((leftSibling->blocks)
                              , (x->blocks)
...
//     insertBit(true, i);
    ...
    SBVNode *x = getNodeWithIthBit(i,infos);
    i -= infos[POSITIONS]+1;
    if (! (x->blocks[i>>log_bs] & (1 << (i&(block_size-1))))) { // bit at pos. i 
      x->myRank++;
      x->subtreeRank++;
      updateCountersOnPathToRoot(x);
    }
    ...
    if (x->myPositions == superblock_size) {
      ...
      if (!  merge_right && !merge_left ) {
        ...
        if (x->getRight() != getNil()) {
          y = (SBVNode*) this->treeMinimum(x->getRight());
          y->setLeft(new_node);
        } else {
          y=x;
          y->setRight(new_node);
...
//         cout << "* New node" << endl;
      ...
      if (merge_right || new_right) {
        ...
        array_copyright((rightSibling->blocks)
                        , (rightSibling->blocks)
                        , nb_shift, rightSibling->myPositions);
        rank = array_copyleft((rightSibling->blocks)
                              , (x->blocks)
                              , superblock_size - nb_shift, nb_shift);

//         cout << " Put " << nb_shift << " bits in right sibling. RS had " << rightSibling->myPositions << " (now " << rightSibling->myPositions+nb_shift << "), current node had " << x->myPositions << " (now " << x->myPositions-nb_shift << ")" << endl;
        ...
        rank = array_copyright((leftSibling->blocks)
                               , (x->blocks)
                               , leftSibling->myPositions, nb_shift);
        array_copyleft((x->blocks)
                       , (x->blocks)
                       , nb_shift, x->myPositions-nb_shift);
//         cout << " Put " << nb_shift << " bits in left sibling. RS had " << leftSibling->myPositions << " (now " << leftSibling->myPositions+nb_shift << "), current node had " << x->myPositions << " (now " << x->myPositions-nb_shift << ")" << endl;
        x->myPositions -= nb_shift;
        x->myRank -= rank;
    ...
    ulong current_block = i>>log_bs;
    ulong current_position = i & (block_size-1);
